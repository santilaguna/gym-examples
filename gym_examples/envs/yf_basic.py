...
class YFBasic(gym.Env):
    ...
    def step(self, action):
        ...
        elif self.eval_env in {"long_train", "val"}:
            date = data.iloc[self.current_step]["Date"]
            done = date >= self.test_init_date
        else:
            done = self.current_step + 1 >= len(data)  # Terminate when data ends
        ...
        for symbol in self.stock_symbols:
            if symbol in self.stock_data:
                ...
                if self.current_step <= len(data):
                    ...
                    raise Exception(f"Current step {self.current_step} is greater than data length {len(data)}")
            else:
